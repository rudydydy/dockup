...
import DeploymentLogs from "./components/deployment_logs";

let deployment_form_container = document.getElementById(
  "deployment_form_container"
);
...
const Deployment = {
  ...
  mountDeploymentForm: (elementId, container_specs_json) => {
    ...
    ReactDOM.render(
      <DeploymentForm containerSpecs={container_specs} />,
      element
    );
  },
  ...
  mountDeploymentLogs: (elementId, container_id) => {
    let element = document.getElementById(elementId);
    let id = JSON.parse(container_id);
    ReactDOM.render(<DeploymentLogs containerId={id} />, element);
  }
};
